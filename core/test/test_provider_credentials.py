...
LIVE_LOOKING_GOOGLE_KEY = "AIzaSyA1234567890abcdefghijklmnopqrstuv"


def test_google_style_api_key_is_not_treated_as_placeholder(monkeypatch) -> None:
    monkeypatch.setenv("GOOGLE_API_KEY", LIVE_LOOKING_GOOGLE_KEY)
...
def test_load_env_file_syncs_antigravity_aliases(tmp_path, monkeypatch) -> None:
    ...
    env_file.write_text(f"ANTIGRAVITY_API_KEY={LIVE_LOOKING_GOOGLE_KEY}\n", encoding="utf-8")

    load_env_file(str(env_file), override=True)

    assert os.getenv("ANTIGRAVITY_API_KEY") == LIVE_LOOKING_GOOGLE_KEY
    assert os.getenv("GEMINI_API_KEY") == LIVE_LOOKING_GOOGLE_KEY
    assert os.getenv("GOOGLE_API_KEY") == LIVE_LOOKING_GOOGLE_KEY
...
def test_load_env_file_syncs_github_aliases(tmp_path, monkeypatch) -> None:
    ...
    env_file = tmp_path / ".env.bridge"
    env_file.write_text("GITHUB_API=github_pat_test_token\n", encoding="utf-8")

    load_env_file(str(env_file), override=True)

    assert os.getenv("GITHUB_API") == "github_pat_test_token"
    assert os.getenv("GITHUB_API_KEY") == "github_pat_test_token"
    assert os.getenv("GITHUB_TOKEN") == "github_pat_test_token"
    assert os.getenv("GH_TOKEN") == "github_pat_test_token"
    assert os.getenv("HOST_BRIDGE_GH_TOKEN") == "github_pat_test_token"
...
def test_load_env_file_syncs_openai_codex_sale_aliases(tmp_path, monkeypatch) -> None:
    ...
    env_file = tmp_path / ".env.bridge"
    env_file.write_text("CODEX_SALE_API_KEY=sk-clb-test-token\nCODEX_SALE_BASE_URL=https://codex.sale\n", encoding="utf-8")

    load_env_file(str(env_file), override=True)

    assert os.getenv("OPENAI_API_KEY") == "sk-clb-test-token"
    assert os.getenv("CODEX_SALE_API_KEY") == "sk-clb-test-token"
